...
The plugin Stop hook in `hooks/hooks.json` runs `node "${CLAUDE_PLUGIN_ROOT}/scripts/goal-helper.mjs" stop-hook`. While a goal is active, that hook blocks stopping and asks you to continue. If the soft token budget or deadline has elapsed, the hook marks the goal `budget_limited` or `deadline_limited` and allows Stop until the user extends the relevant limit.
